...
void main() {
  ...
  test(
    'initialize echoes the client protocol version and advertises tools',
    () async {
  ...
  test(
    ...
    () async {
      ...
      expect(semanticMemory['source'], 'episode');
      expect(semanticMemory['matchReasons'], contains(contains('semântica')));
      ...
      expect(temporalMemory['sourceId'], id);
      expect(temporalMemory['matchReasons'], contains(contains('temporal')));
    },
  );
  ...
  test(
    'create_snippet then get_snippet round-trips through JSON content',
    () async {
      final createResponse = await server.handleMessage(
        _call('create_snippet', {
          'title': 'Reverse a string',
          'content': 'a.split("").reversed',
        }),
      );
      final created =
          jsonDecode(_text(createResponse!)) as Map<String, dynamic>;
      expect(created['title'], 'Reverse a string');

      final getResponse = await server.handleMessage(
        _call('get_snippet', {'id': created['id']}),
      );
  ...
  test('search_snippets finds by keyword', () async {
    ...
    final response = await server.handleMessage(
      _call('search_snippets', {'query': 'reverse'}),
    );
  ...
  test('update_snippet changes only the given fields', () async {
    ...
    await server.handleMessage(
      _call('update_snippet', {'id': id, 'title': 'Bar'}),
    );

    final getResponse = await server.handleMessage(
      _call('get_snippet', {'id': id}),
    );
    final fetched = jsonDecode(_text(getResponse!)) as Map<String, dynamic>;
  ...
  test('delete_snippet removes it', () async {
    ...
    final deleteResponse = await server.handleMessage(
      _call('delete_snippet', {'id': id}),
    );
  ...
  test(
    'calling an unknown tool is a tool-level error, not a JSON-RPC error',
    () async {
  ...
  test(
    'ask_kango_ltm returns activity and summaries within the parsed range',
    () async {
      ...
      expect(
        (result['activities'] as List).single['windowTitle'],
        'today-work.dart',
      );
      expect(
        (result['summaries'] as List).single['content'],
        'Recap of today.',
      );
    },
  );
  ...
  test(
    'ask_kango_ltm with keywords full-text-searches activity in range',
    () async {
      final now = DateTime.now();
      await database.logActivity(
        ActivitiesCompanion.insert(
  ...
  test(
    ...
    () async {
      ...
      final askResponse = await server.handleMessage(
        _call('ask_kango_ltm', {'query': 'today'}),
      );
  ...
  test(
    'semantic search without a semantic search instance reports an error',
    () async {
      final serverWithoutSemantic = KangoMcpServer(
        snippets: SnippetService(repository: SqliteSnippetRepository(database)),
        memory: MemoryService(
          database: database,
